...
Спецификацией считается любой из глаголов understudy, принимающих
call-замыкание, в обоих написаниях: свободные функции `when()`, `expect()`,
`expectSequence()` и `verify()`, те же имена на `Understudy::` и читатели,
существующие только там, — `calls()`, `lastCall()` и `verifySequence()`.
Учитывается каждое замыкание протокола: матчер в третьем шаге
`expectSequence()` читается ровно так же, как матчер в `when()`.
...
Форма описывает ровно то, что строит ядро. Параметр типа
`BookRepository&Auditor` — это ОДИН дубль, стоящий за оба контракта, поэтому
ключ сохраняет пересечение и обе половины остаются вызываемыми. Параметр типа
`BookRepository|Clock` заставляет ядро отказаться от класса целиком
(`CannotWire`: выбрать один из двух было бы догадкой), поэтому вызов не
описывается никакой формой и остаётся собственное объявление ядра
`array{sut: object, doubles: array<string, object>}`. Динамический class-string
не трогается по той же причине.
...
| `understudy.matcherLeak` | Матчер вне спецификации и вне замыкания — там он доходит до кода как значение. Матчер, поднятый в переменную, положенный в свойство или написанный в замыкании, которое передадут позже, таковым не считается: замыкание ещё не исполнялось, а спецификация, которая его исполнит, находится в другом месте |
...
Эти идентификаторы стабильны. Именно их потребитель выписывает в свой
`phpstan.neon`, и переименование любого молча сломало бы чью-то запись в
`ignoreErrors`; новое правило получает новый идентификатор, а не переиспользует
существующий. **Формулировка** сообщения стабильной не является, патч-релиз
вправе её переписать: сопоставлять нужно идентификатор, а не предложение.
