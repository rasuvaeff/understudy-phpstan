...
{
    /**
     * The one core static that takes a call closure first and is still not a
     * specification: `scope()` runs the test's own code, so claiming it would
    ...
    public static function staticProvider(): iterable
    {
        ...
        // Deliberately not a verb: `scope()` takes a callable first too,
        // but it is the test's own code, and claiming it would silence
        // every matcher diagnostic inside the block.
     ...
     * added to the core now fails this build instead of turning into a false
     * report in a consumer's project.
     */
     ...
     */
    public function theDeliberateNonVerbsStillLookLikeVerbs(): void
    {
